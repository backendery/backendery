...
const VirtualScene: React.FC = () => {
  ...
  const setSpaceAndZoomIt = (event: MouseEvent): void => {
    ...
    const xPosition = Number.parseInt(target.getAttribute('data-x-position') || '0') * -1;
    const yPosition = Number.parseInt(target.getAttribute('data-y-position') || '0');
   ...
   */
  const moveUp = (): void => {
    virtualSceneWrapperRef.current && setPositionY((previousPositionY) => previousPositionY + 1);
  };
  ...
  const moveRight = (): void => {
    virtualSceneWrapperRef.current && setPositionX((previousPositionX) => previousPositionX - 1);
  };
  ...
  const toggleNavigationVisibility = (isHide: boolean): void => {
    if (spacesNavigationRef.current) {
      for (const navigateItem of spacesNavigationRef.current) {
  ...
  const toggleCaptionVisibility = (isHide: boolean): void => {
    if (spacesCaptionRef.current) {
      for (const captionItem of spacesCaptionRef.current) {
  ...
  const zoomIn = (): void => {
    if (spacesRef.current) {
      for (const spaceItem of spacesRef.current) {
  ...
  useEffect(() => {
    ...
    const navigationMap = [
      { elements: document.querySelectorAll('.js-up'), handler: moveUp },
      {
        elements: document.querySelectorAll('.js-right'),
        handler: moveRight,
      },
      {
        elements: document.querySelectorAll('.js-down'),
        handler: moveDown,
      },
      {
        elements: document.querySelectorAll('.js-left'),
        handler: moveLeft,
      },
    ];

    // Adding click event listeners to each navigation element
    for (const { elements, handler } of navigationMap) {
      for (const navigateItem of Array.from(elements)) {
        navigateItem.addEventListener('click', handler);
      }
    }
    ...
    return () => {
      ...
      for (const { elements, handler } of navigationMap) {
        for (const navigateItem of Array.from(elements)) {
          navigateItem.removeEventListener('click', handler);
        }
      }
    };
  }, []);
