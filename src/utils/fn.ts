...
export function getElementHeight(selector: string): number {
  ...
  const heightString: string = window.getComputedStyle(elt).getPropertyValue('height');

  return Number.parseInt(heightString, 10);
}
...
export function getElementWidth(selector: string): number {
  ...
  const widthString: string = window.getComputedStyle(elt).getPropertyValue('width');

  return Number.parseInt(widthString, 10);
}
...
export function hasClass(element: HTMLElement, cls: string): boolean {
  return Boolean(new RegExp('(\\s|^)' + cls + '(\\s|$)').test(element.className));
}
 ...
 * @param {number} [timeout] The time in milliseconds to wait before executing the function. Defaults to 300ms.
 * @example
 * ```tsx
...
export function runWithTimeout(
  timeoutRef: import('react').MutableRefObject<null | number>,
