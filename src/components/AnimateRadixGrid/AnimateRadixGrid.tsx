...
type AnimateRadixGridProps = {
   ...
   */
  readonly unreachableCells?: Array<[number, number]>;
};
 ...
 */
const AnimateRadixCell: FC<AnimateRadixCellProps> = memo(({ isUnreachable, symbol }) => {
 ...
 * update process.
 */
const AnimateRadixGrid: FC<AnimateRadixGridProps> = (props) => {
  /**
   ...
   */
  const [cells, setCells] = useState<string[]>(Array.from({ length: props.rows * props.cols }, () => ''));
  /**
  ...
  if (props.unreachableCells) {
    for (const [row, col] of props.unreachableCells) {
  ...
  useEffect(() => {
    if (!isHydrate || !isVisible) {
      return;
    } // Don't animate if the component is not visible
